...
The system SHALL register an `AbstractMongoEventListener` that intercepts `BeforeConvertEvent`. For each entity type with `@Encrypted` fields, it SHALL inspect the BSON Document, locate sub-documents with `_e: 1`, decrypt the `c` field using `CryptoCodec`, parse the result according to `_t` type marker, and replace the sub-document with the original typed value in the Document.
