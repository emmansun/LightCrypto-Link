...
The system SHALL provide a `FieldCryptoService` Spring Bean (located in `lcl-spring-boot-starter` module) that exposes `Object decryptDocument(Object rawDocument, Class<?> entityClass)`. The method SHALL iterate over all `@Encrypted` fields declared in `entityClass`, locate encrypted sub-documents in `rawDocument`, decrypt them, and replace the sub-document with the plaintext value in-place. The method SHALL return the same `rawDocument` instance for fluent/chain usage. Document field access SHALL be delegated to `DocumentAccessor`; structured value deserialization SHALL be delegated to `StructuredValueCodec`.
...
`CryptoMappingMongoConverter.decryptFields()` (located in `lcl-adapter-mongodb`) SHALL delegate its decryption logic to `FieldCryptoService.decryptDocument()`. The Converter retains responsibility for *when* to decrypt (during `read()` and `project()`); the Service owns *how* to decrypt.
