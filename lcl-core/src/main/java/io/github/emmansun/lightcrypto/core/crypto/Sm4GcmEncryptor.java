...
/**
 * SM4-GCM encryptor with AAD support (Bouncy Castle).
 *
 * @since 1.0.0
 */
public class Sm4GcmEncryptor implements SymmetricEncryptor {
    ...
    public byte[] encrypt(byte[] key, byte[] iv, byte[] plaintext, byte[] aad) {
        try {
            ...
            throw new EncryptionException("SM4-GCM encryption failed", e);
        }
    }
    ...
    public byte[] decrypt(byte[] key, byte[] iv, byte[] ciphertext, byte[] aad) {
        try {
            ...
            throw new CryptoAuthenticationException(
                    "SM4-GCM authentication failed", e, null, 0, algorithmId().name());
        } catch (Exception e) {
