 ...
 * Uses HKDF-derived per-namespace blind index keys via BlindIndexEngine.
 */
public class CryptoMongoQueryCreator {
    ...
    private void rewriteCriteriaDocument(Document doc, List<EncryptedFieldMetadata> fields) {
        ...
        for (String key : keys) {
            ...
            // Simple 'is' query
            String hashed = hashValue(matched, value);
            doc.put(matched.bsonFieldName() + ".b", hashed);
            doc.remove(key);
        }
    }

    private String hashValue(EncryptedFieldMetadata meta, Object value) {
