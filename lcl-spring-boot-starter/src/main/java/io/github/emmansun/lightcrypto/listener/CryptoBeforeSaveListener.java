...
public class CryptoBeforeSaveListener {
        ...
        try {
                ...
                if (isLeaf) {
                    if (meta.wholeObject()) {
                        ...
                        if (rawBsonValue instanceof Document rawDoc) {
                            ...
                            bsonContext.put(segment, buildEncryptedSubDoc(meta, serialized, "DOC"));
                        }
                        return;
                    }
                    bsonContext.put(segment, buildEncryptedSubDoc(meta, value));
                    return;
                }
            ...
            if (pathType == PathSegmentType.LIST_ITER) {
                ...
                if (isLeaf && meta.wholeObject()) {
                    ...
                    bsonContext.put(segment, buildEncryptedSubDoc(meta, serialized, "COL"));
                    return;
                }
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (isLeaf) {
                        bsonList.set(i, buildEncryptedSubDoc(meta, item));
                    } else {
            ...
            if (pathType == PathSegmentType.MAP_ITER) {
                ...
                if (isLeaf && meta.wholeObject()) {
                    ...
                    bsonContext.put(segment, buildEncryptedSubDoc(meta, serialized, "MAP"));
                    return;
                }
                ...
                for (Map.Entry<?, ?> entry : map.entrySet()) {
                    ...
                    if (isLeaf) {
                        mapDoc.put(keyName, buildEncryptedSubDoc(meta, item));
                    } else {
    ...
    private Document buildEncryptedSubDoc(EncryptedFieldMetadata meta, Object value) {
        byte[] serialized = typeSerializer.serialize(value);
        return buildEncryptedSubDoc(meta, serialized, resolveTypeMarker(meta.javaType()));
    }
    ...
    private Document buildEncryptedSubDoc(EncryptedFieldMetadata meta,
        ...
        if (meta.blindIndex()) {
            ...
            String blindIndex = engine.computeBlindIndex(
